...
func _input_event(viewport, event, shape_idx):
	if Input.is_action_pressed("click"):
		...
		print(Global.nearby_objects)
		if Global.nearby_objects.has(self.name):
			_dialog_start()
			met_character()
		else:
			print("NOT NEAR ")
			show_textbox("You need to be closer to talk to this person.")
...
func show_textbox(message: String):
	...
	active_lines = [message]
	add_child(textbox_scene)
	textappear.show_clue_container(false)
	textappear._on_clue_clicked_text.connect(_on_clue_clicked)
	textappear.clear_clues()
	textappear.add_text(message)
	is_dialog_active = true
